...
export async function GET(req: Request) {
  ...
  const ledgerRows = [
    ...entries.map((entry) => ({
      ...
      employeeId: entry.worklog.user.id,
      employeeName: entry.worklog.user.name ?? entry.worklog.user.email,
      employeeEmail: entry.worklog.user.email,
