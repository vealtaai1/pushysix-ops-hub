...
export function ManagementClientsClient({ initialClients }: { initialClients: ManagementClientRow[] }) {
  const [selected, setSelected] = React.useState<ManagementClientRow | null>(null);
          ...
          <div className="flex w-full max-w-xl max-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-xl bg-white shadow-xl">
            ...
            <div className="p-4 overflow-auto">
              <form action={updateClientRetainerBasics} className="grid gap-3">
                ...
                <div className="text-xs text-zinc-500">Note: after saving, refresh the Clients page to see the updated values.</div>

                <div className="mt-2 rounded-md border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-600">Deleting clients is admin-only.</div>
              </form>
            </div>
          </div>
        </div>
